...
import { assistFromContext, assistQuery, attachNotebookFile, createNotebookItem, deleteNotebookItemFile, deleteNotebookItem, getMeCapabilities, getNotebookItemChunks, getNotebookItemParsedPreview, getNotebookIndexStatus, listNotebookItemFiles, listNotebookItems, retryNotebookIndexJob, syncPull, syncPush, updateNotebookItem } from './routes/notebook.js';
...
function isAllowedCorsOrigin(origin) {
    ...
    if (corsOrigins.includes(origin))
        return true;
    if (/^https:\/\/[a-z0-9-]+\.gotradetalk-ui\.pages\.dev$/i.test(origin))
        return true;
    return false;
}
app.use(cors({
