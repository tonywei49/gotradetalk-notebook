...
function wrapProviderError(provider, message) {
    const detail = message || 'MODEL_ERROR';
    if (detail.startsWith('MODEL_ERROR(provider='))
        return detail;
...
export async function getNotebookAiConfig(companyId) {
    ...
    return {
        ...
        visionBaseUrl: normalizeApiBaseUrl(String(getConfigValue('vision_base_url', 'notebook_ai_vision_base_url')
            || getConfigValue('chat_base_url', 'notebook_ai_chat_base_url')
            || data?.notebook_ai_vision_base_url
            || data?.notebook_ai_chat_base_url
            || defaultBaseUrl)),
        visionApiKey: String(getConfigValue('vision_api_key', 'notebook_ai_vision_api_key')
...
export async function generateAssistAnswer(config, query, contextBlocks, responseLanguage, promptContext) {
    ...
    const systemPrompt = [
        '你是一名业务助理。请根据知识库召回内容，结合客户当前问题与前文背景，生成一段可直接发送给客户的回复。',
        '只能依据知识库内容作答，不可补充知识库没有的信息，不可自行推测。',
        '直接进入主题，禁止使用“你好”“您好”“谢谢”等客套开场。',
        '回复要简洁、干练、自然，像业务同事真实会发出的消息。',
        '目标是尽量在5句以内把客户问题解释清楚；能更短就更短，但要把关键信息说完整。',
        '如果结论明确，就直接说明结论；如果需要额外确认，只补充最关键的确认点，不要展开过多说明。',
        '不要输出标题、标签、编号、项目符号、引号、说明文字或格式标记。',
    ...
    const userPrompt = [
        ...
        '知識庫引用內容：',
        contextText,
        '請根據以上內容，直接輸出一段可發送給客戶的最終回覆。',
        '要求：',
        '- 直接回答，不要客套開場',
        '- 簡潔幹練',
        '- 盡量控制在 5 句以內，把問題解釋清楚',
        '- 不要解釋你的依據，不要重述知識庫原文，不要加多餘鋪陳',
        languageInstruction
    ].join('\n');
    let resp;
    try {
