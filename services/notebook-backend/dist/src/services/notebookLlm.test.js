...
test('generateAssistAnswer keeps the full reply body when the model starts with a greeting', async () => {
    ...
    try {
        const result = await generateAssistAnswer(TEST_CONFIG, '如果我給30%預付款，可以做到25天嗎？', [{ source: '[交期政策|S1]', text: '若支付30%预付款，可评估压缩到25天内，但需确认排产。' }], 'zh-CN', { currentQuestion: '如果我給30%預付款，可以做到25天嗎？', priorMessages: [] });
        assert.equal(result.referenceAnswer, '你好！\n如果您支付30%预付款，交期可以评估压缩到25天内，但仍需以排产与物料情况确认。');
    }
    finally {
...
test('generateAssistAnswer strips optional answer labels when present', async () => {
    ...
    try {
        const result = await generateAssistAnswer(TEST_CONFIG, '可以25天出貨嗎？', [{ source: '[交期政策|S1]', text: '25天交付需确认排产与物料。' }], 'zh-TW', { currentQuestion: '可以25天出貨嗎？', priorMessages: [] });
...
test('generateAssistAnswer sends concise no-greeting prompt instructions', async () => {
    ...
    try {
        await generateAssistAnswer(TEST_CONFIG, '如果我給30%預付款，可以做到25天嗎？', [{ source: '[交期政策|S1]', text: '若支付30%预付款，可评估压缩到25天内，但需确认排产。' }], 'zh-CN', { currentQuestion: '如果我給30%預付款，可以做到25天嗎？', priorMessages: [] });
