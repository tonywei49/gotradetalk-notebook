...
test('integration: xlsx multi-sheet keeps per-block locators', async () => {
    ...
    assert.deepEqual(parsed.segments?.map((segment) => segment.sourceLocator), ['sheet:Orders row:1-2', 'sheet:Orders row:4-5', 'sheet:Parts row:1-2']);
});
test('integration: xlsx cleanup preserves original row locators after repeated edge removal', async () => {
    ...
    assert.deepEqual(parsed.segments?.map((segment) => segment.sourceLocator), ['sheet:One row:2-3', 'sheet:Two row:2-3']);
...
test('integration: xlsx parser trims inflated worksheet range before extraction', async () => {
    ...
    const xlsxBuffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
    const parsed = await parseDocument(xlsxBuffer, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'inflated-range.xlsx');
    assert.equal(parsed.sourceType, 'xlsx');
