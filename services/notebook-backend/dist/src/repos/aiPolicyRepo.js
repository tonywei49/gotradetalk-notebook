...
export async function getPlatformAiSettings(capabilityType) {
    const result = await dbQuery(`select capability_type::text as capability_type,
            managed_by_platform,
            config
       from public.platform_ai_settings
      where capability_type = $1
      limit 1`, [capabilityType]);
    return result.rows[0] || null;
}
export async function getSubjectAiPolicy(params) {
    const result = await dbQuery(`select subject_type::text as subject_type,
      ...
      limit 1`, [params.subjectType, params.subjectId, params.capabilityType]);
    return result.rows[0] || null;
}
export async function incrementSubjectAiPolicyUsage(params) {
    const monthKey = new Date().toISOString().slice(0, 7);
    await dbQuery(`update public.subject_ai_policies
      ...
      where subject_type = $1
        and subject_id = $2
        and capability_type = $3`, [params.subjectType, params.subjectId, params.capabilityType, monthKey]);
}
