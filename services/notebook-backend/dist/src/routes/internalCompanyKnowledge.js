import { createNotebookItem, getNotebookChunkStatsByItem, getLatestChunkSettingsByItem, getLatestIndexJobByItem, getNotebookItemByCompany, listNotebookChunksByItem, listNotebookItems, updateNotebookItemByOwner } from '../repos/notebookRepo.js';
...
function resolveActorLabel(req) {
    const raw = String(req.headers['x-actor-label'] || req.body?.actor_label || '').trim();
    if (!raw)
        return 'company_admin';
    return raw.slice(0, 64);
}
function resolveActorProfileId(req, companyId) {
    const headerId = String(req.headers['x-actor-profile-id'] || '').trim();
    if (headerId)
        return headerId;
    const bodyId = String(req.body?.actor_profile_id || '').trim();
    if (bodyId)
        return bodyId;
    return companyId;
}
...
async function loadUploaderMap(companyId, ownerUserIds) {
    const ids = Array.from(new Set(ownerUserIds.map((id) => String(id || '').trim()).filter(Boolean)));
    if (ids.length === 0)
        return new Map();
    const result = await dbQuery(`select id::text as id, user_local_id, auth_user_id
...
async function ensureActorProfile(req, companyId) {
...
export async function listInternalCompanyKnowledgeItems(req, res) {
    if (!authorized(req)) {
...
export async function createInternalCompanyKnowledgeItem(req, res) {
    if (!authorized(req)) {
        return res.status(401).json({ code: 'UNAUTHORIZED', message: 'Invalid internal secret' });
    }
    const companyId = resolveCompanyId(req);
...
export async function deleteInternalCompanyKnowledgeItem(req, res) {
    if (!authorized(req)) {
        return res.status(401).json({ code: 'UNAUTHORIZED', message: 'Invalid internal secret' });
    }
    const companyId = resolveCompanyId(req);
...
export async function offlineInternalCompanyKnowledgeItem(req, res) {
    if (!authorized(req)) {
        return res.status(401).json({ code: 'UNAUTHORIZED', message: 'Invalid internal secret' });
    }
    const companyId = resolveCompanyId(req);
...
export async function retryInternalCompanyKnowledgeIndex(req, res) {
    if (!authorized(req)) {
        return res.status(401).json({ code: 'UNAUTHORIZED', message: 'Invalid internal secret' });
    }
    const companyId = resolveCompanyId(req);
...
export async function previewInternalCompanyKnowledgeItem(req, res) {
    if (!authorized(req)) {
        return res.status(401).json({ code: 'UNAUTHORIZED', message: 'Invalid internal secret' });
    }
    const companyId = resolveCompanyId(req);
